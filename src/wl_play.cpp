...
void PollKeyboardMove(void)
{
	...
	if (cmd.buttonstate[bt_moveforward])
		cmd.controly -= delta;
	else if (cmd.buttonstate[bt_movebackward])
		cmd.controly += delta;
	...
	else if (cmd.buttonstate[bt_straferight])
		cmd.controlstrafe += delta;
	#else
	int ndelta = ((!alwaysrun && cmd.buttonstate[bt_run]) || (alwaysrun && !cmd.buttonstate[bt_run]))+1;
	if (cmd.buttonstate[bt_turnleft])
		cmd.controlx -= delta;
	if (cmd.buttonstate[bt_turnright])
		cmd.controlx += delta;
	...
	if (cmd.buttonstate[bt_moveforward])
		cmd.controlsy -= ndelta;
	if (cmd.buttonstate[bt_movebackward])
		cmd.controlsy += ndelta;

	if (cmd.buttonstate[bt_strafeleft])
		cmd.controlmstrafe -= ndelta;
	if (cmd.buttonstate[bt_straferight])
		cmd.controlmstrafe += ndelta;

	if (!cmd.buttonstate[bt_moveforward] && !cmd.buttonstate[bt_movebackward])
	{
		if (cmd.controlsy > 0)
			cmd.controlsy--;
		else if (cmd.controlsy < 0)
			cmd.controlsy++;
	};

	if (!cmd.buttonstate[bt_strafeleft] && !cmd.buttonstate[bt_straferight])
	{
		if (cmd.controlmstrafe > 0)
			cmd.controlmstrafe--;
		else if (cmd.controlmstrafe < 0)
			cmd.controlmstrafe++;
	};

	if (cmd.controlsy < -100)
		cmd.controlsy = -100;
	if (cmd.controlsy > 100)
		cmd.controlsy = 100;

	if (cmd.controlmstrafe < -100)
		cmd.controlmstrafe = -100;
	if (cmd.controlmstrafe > 100)
		cmd.controlmstrafe = 100;

	cmd.controly = cmd.controlsy;
	cmd.controlstrafe = cmd.controlmstrafe;
	#endif
...
void CheckKeys(void)
{
	...
	if (automap != AMA_Normal && changeSize)
	{
		...
		else if (!Keyboard[sc_Equals] && Keyboard[sc_Minus] && !(gameinfo.LockBorder && viewsize <= 20))
			NewViewSize(viewsize - 1);
		if (Keyboard[sc_Equals] || Keyboard[sc_Minus])
		{
